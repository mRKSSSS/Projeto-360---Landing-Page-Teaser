...
$(document).ready(function(){
    ...
    $('.icon-cross, .steering-wheel-left').mouseenter(function(){  
        $(this).siblings('.steering-wheel').addClass('turn-left');
        $(this).siblings('.icon-cross').addClass('hover-click');
    });
    $('.icon-cross, .steering-wheel-left').mouseleave(function(){
        $(this).siblings('.steering-wheel').removeClass('turn-left');
        $(this).siblings('.icon-cross').removeClass('hover-click');
    });
    $('.icon-checkmark, .steering-wheel-right').mouseenter(function(){
        $(this).siblings('.steering-wheel').addClass('turn-right');
        $(this).siblings('.icon-checkmark').addClass('hover-click');
    });
    $('.icon-checkmark, .steering-wheel-right').mouseleave(function(){
        $(this).siblings('.steering-wheel').removeClass('turn-right');
        $(this).siblings('.icon-checkmark').removeClass('hover-click');
    });
    ...
    $('.steering-wheel-left, .steering-wheel-right').click(function(){
        ...
        if($(this).hasClass('steering-wheel-left')){
            ...
            $(this).siblings('.icon-cross').addClass('clicked');
            $(this).siblings('.icon-checkmark').addClass('not-clicked');
        }else if($(this).hasClass('steering-wheel-right')){
            $(this).prev().unbind('click mouseenter mouseleave');
            $(this).siblings('.icon-cross').addClass('not-clicked');
            $(this).siblings('.icon-checkmark').addClass('clicked');
        };
        // $(this).parent().parent().children('.not-got-it-button').hide();
        // $(this).parent().parent().children('.scroll-down').show();
        // $(this).parent().parent().next().addClass('show');
    ...
    $('.finish').click(function(){
        ...
        const socialNetworks = '<div class="social-network">' +
